...
def dashboard(request):
    ...
    if not hasattr(request.user, 'userspotifylink'):
        messages.error(request, _("Please link your Spotify account to proceed."))
    ...
    wraps_with_serialized_data = [
        {
            "id": wrap.id,
            "wrap_name": _(wrap.wrap_name),
            "wrap_type": _(wrap.wrap_type),
...
def delete_wrap(request, wrap_id):
    if request.method == 'POST':
        ...
        messages.success(request, _("Your wrap has been deleted successfully."))
        return redirect('wraps:dashboard')  # Redirect to the dashboard after deletion
    return HttpResponse(_("Invalid request method"), status=405)
